...
String excelLocation = 'Daftar Tiket\\Detail Tiket\\TambahNote - add.xlsx'
String sheetName = 'TambahNote-add'

List<HashMap> listHashMapNote = handleTestData.readTestData(excelLocation, sheetName, true)

for (int i = 0; i < listHashMapNote.size(); i++) {
	try {
		HashMap getHashMapNote = listHashMapNote.get(i)
		KeywordUtil.logInfo(getHashMapNote.toString())
		String testDataNumber = getHashMapNote.get('TD')
		String ticket_number = getHashMapNote.get('ticket_number')
		String note = getHashMapNote.get('note')
		
		detailtiket.cariTiket(ticket_number)
		detailtiket.detailTiket(ticket_number)
		detailtiket.tambahNote(note)
