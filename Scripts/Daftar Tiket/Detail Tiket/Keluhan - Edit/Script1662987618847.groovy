...
String excelLocation = 'Daftar Tiket\\Detail Tiket\\keluhan - edit.xlsx'
String sheetName = 'keluhan-edit'

List<HashMap> listHashMapKeluhan = handleTestData.readTestData(excelLocation, sheetName, true)

for (int i = 0; i < listHashMapKeluhan.size(); i++) {
	try {
		HashMap getHashMapKeluhan = listHashMapKeluhan.get(i)
		KeywordUtil.logInfo(getHashMapKeluhan.toString())
		String testDataNumber = getHashMapKeluhan.get('TD')
		String ticket_number = getHashMapKeluhan.get('ticket_number')
		
		detailtiket.cariTiket(ticket_number)
		detailtiket.detailTiket(ticket_number)
		detailtiket.editKeluhan(getHashMapKeluhan)
