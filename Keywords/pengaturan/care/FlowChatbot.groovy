...
public class FlowChatbot {
	...
	public void setPesanSolusi(HashMap hashMapsetFlowChatbot) {
		...
		String linkURL = hashMapsetFlowChatbot.get('Link URL')
				...
				for(int j = 0; j < indeks.size(); j++) {
					...
					if(!linkURL.equalsIgnoreCase('')) {
						...
						for(int k = 0; k < url.size(); k++) {
							...
							GeneralAction.clickElement(findTestObject('Object Repository/Pengaturan/Flow Chatbot/Default Message/btnAddLink', [('indeks') : indeks[i]]))
							GeneralAction.clickElementAndType(findTestObject('Object Repository/Pengaturan/Flow Chatbot/Default Message/input_LinkURL', [('indeks') : indeks[i]]), url[i])
						}
					}
				}
			}
